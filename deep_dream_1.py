...
def main():
    ...
    with tf.gfile.FastGFile(os.path.join(data_dir, model_fn), 'rb') as \
        f:
    ...
    feature_nums = [int(graph.get_tensor_by_name(name + ':0'
                    ).get_shape()[-1]) for name in layers]
 ...
 # ###########################################################
    ...
    # split the image into a number of octaves
        ...
        for filename in glob.glob('/home/rahuldeo/sirajology/deep_dream_1/video/*.jpg'
            ...
            for _ in range(octave_n - 1):
                hw = img.shape[:2]
                lo = resize(img, np.int32(np.float32(hw)
                            / octave_scale))
                hi = img - resize(lo, hw)
                img = lo
                octaves.append(hi)
        ...
        # generate details octave by octave
            ...
            filename = filename.replace("output_", "output2_")
            print(filename)
            showarray(img / 255.0, filename)
